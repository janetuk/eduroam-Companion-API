...
for($i = 0; $i<count($sites); $i++){
	...
	$removeTags = array();	//Array of tags that are older than 6 months and to be removed
	...
	$boundary = mktime(date('G'), date('i'), date('s'), date('n') - 6, date('j'));	//UNIX timestamp of the time 6 months ago
 	//$boundary = intval($boundary);
	...
	while($stmt->fetch()){
		...
		//$results["tagdate"] = intval($results["tagdate"]);
		
		//if tags are older than 6 months, remove them from the database
		if($results["tagdate"] < $boundary){
		
			echo "Tag time: " . $results["tagdate"] . " - boundary: " . $boundary . " \n";
			$removeTags[] = $result["id"];
	...
	//Remove all the tags that are older than 6 months
	foreach($removeTags as $toBeDeleted){
	
			$query = "DELETE FROM APTags WHERE Tag = ?";
			$stmt = $mysqli->prepare($query);

			$stmt->bind_param("i", $toBeDeleted);
			echo $mysqli->error . "\n";
			echo "tag is: " . $toBeDeleted . " \n";

			if(!$stmt->execute()){
				if($REPORT_ERRORS == 1){
					die('{"error":{"code":41, "msg":"Problem with SQL - deleting old tag"}}');
				}
			}
			
			$stmt->close();

			//REMOVE ALL OLD TAGS HERE
			$query = "DELETE FROM Tags WHERE id = ?";
			$stmt = $mysqli->prepare($query);
			$stmt->bind_param("i", $toBeDeleted);
			if(!$stmt->execute()){
				if($REPORT_ERRORS == 1){
					die('{"error":{"code":40, "msg":"Problem with SQL - deleting old tag"}}');
				}
			}
			$stmt->close();	
	}
	...
	while(count($tags) > 0){
		...
		if(count($clusters) > 0){
			...
			for($j=1; $j<count($clusters); $j++){
				...
				//print_r($clusters[$j]);
				if($clusters[$j]->getUniqueSize() > $candidateCluster->getUniqueSize()){
...
*/
			...
			if($candidateCluster->getUniqueSize() >= $MINIMUM_NO_OF_TAGS_FOR_CLUSTER){
...
*/
	...
	for($j=0; $j<count($finalClusters); $j++){
		...
		if($cluster->getUniqueSize() >= (10 + $MINIMUM_NO_OF_TAGS_FOR_CLUSTER)){
			//give it a rating of 1
			$cluster->setRating(1);
		} else {
			//give it a rating of uniquesize/10
			$cluster->setRating(($cluster->getUniqueSize() - $MINIMUM_NO_OF_TAGS_FOR_CLUSTER + 1) / 10);
		}
	}
		...
		//print_r($cluster);
			...
			foreach($existingAPs as $ap){
				...
				if(($cluster->getLat() == $ap->getLat()) && ($cluster->getLng() == $ap->getLng()) && ($cluster->getRating() == $ap->getRating())){
					...
					foreach($cluster->getUniqueTags() as $tag){
						$tagid = $tag->getID();
						if(!$stmt->execute()){
							if($REPORT_ERRORS == 1){
...
class Cluster{
	...
	public function getCentre(){
		...
		//average the x,y,z
		$x = $x/count($this->uniqueTags);
		$y = $y/count($this->uniqueTags);
		$z = $z/count($this->uniqueTags);
